...
void InProcTestNode::workerThread(std::promise<std::string>& initPromise) {
  ...
  try {
    ...
    protocol.reset(new CryptoNote::cryptonote_protocol_handler(m_currency, dispatcher, *core, NULL, log));
    p2pNode.reset(new CryptoNote::node_server(dispatcher, *protocol, log));
...
bool InProcTestNode::startMining(size_t threadsCount, const std::string &address) {
  ...
  return core->get_miner().start(addr, threadsCount, boost::thread_attributes());
}
...
bool InProcTestNode::getBlockTemplate(const std::string &minerAddress, CryptoNote::Block &blockTemplate, uint64_t &difficulty) {
  ...
  uint32_t height = 0;
  return core->get_block_template(blockTemplate, addr, difficulty, height, blobdata());
}

bool InProcTestNode::submitBlock(const std::string &block) {
  block_verification_context bvc = boost::value_initialized<block_verification_context>();
  core->handle_incoming_block_blob(block, bvc, true, true);
