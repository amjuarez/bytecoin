...
public:
  ...
  bool waitLastKnownBlockHeightUpdated(std::chrono::milliseconds ms, uint64_t& value) {
    return m_knownHeight.waitFor(ms, value);
  }

  bool waitLocalBlockchainUpdated(std::chrono::milliseconds ms, uint64_t& value) {
    return m_localHeight.waitFor(ms, value);
  }

  uint64_t waitLastKnownBlockHeightUpdated() {
    return m_knownHeight.wait();
  }

  ObservableValue<uint64_t> m_knownHeight;
  ObservableValue<uint64_t> m_localHeight;
