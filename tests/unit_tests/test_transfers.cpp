...
public:
  ...
  void addPaymentAccounts(size_t count) {
    auto viewKeys = generateKeys();
    while (count--) {
      m_accounts.push_back(accountKeysFromKeypairs(viewKeys, generateKeys()));
    }
  }
