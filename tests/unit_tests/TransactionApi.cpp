...
TEST_F(TransactionApi, deserializeTransactionInputMultisignature) {
  ...
  TransactionTypes::InputMultisignature restoredMsig;
  ...
  EXPECT_EQ(restoredMsig.signatures, inputMsig.signatures);
  EXPECT_EQ(restoredMsig.term, inputMsig.term);
}


TEST_F(TransactionApi, deserializeTransactionOutputMultisignature) {
  std::vector<AccountAddress> destinations = {generateAccountKeys().address, generateAccountKeys().address, generateAccountKeys().address};
  ...
  TransactionTypes::OutputMultisignature msigOutput;
  restoredTransaction->getOutput(index, msigOutput);

  EXPECT_EQ(msigOutput.requiredSignatures, REQUIRED_SIGNATURES);
  EXPECT_EQ(msigOutput.amount, AMOUNT);
