...
struct TotalWalletBalance {
  ...
  TotalWalletBalance(IWallet& wallet) 
...
class IntegrationTest : public Tests::Common::BaseFunctionalTest, public ::testing::Test {
public:

  IntegrationTest() : 
    currency(CryptoNote::CurrencyBuilder(log).testnet(true).currency()), 
    BaseFunctionalTest(currency, globalDispatcher, baseCfg),
  ...
  void makeWallets() {
    for (auto& n: inodes) {
      std::unique_ptr<CryptoNote::IWallet> wallet(new CryptoNote::Wallet(m_currency, *n));
  ...
  std::vector<std::unique_ptr<IWallet>> wallets;
  std::vector<std::unique_ptr<WalletObserver>> walletObservers;
};
...
TEST_F(IntegrationTest, BlockPropagationSpeed) {
...

  {
    ...
    for (size_t blockNumber = 0; blockNumber < BLOCKS_COUNT; ++blockNumber) {
      uint64_t localHeight = localObserver.waitLastKnownBlockHeightUpdated();
      uint64_t remoteHeight = 0;
