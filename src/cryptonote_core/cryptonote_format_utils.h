...
bool find_tx_extra_field_by_type(const std::vector<tx_extra_field>& tx_extra_fields, T& field) {
  auto it = std::find_if(tx_extra_fields.begin(), tx_extra_fields.end(),
    [](const tx_extra_field& f) { return typeid(T) == f.type(); });
...
bool check_outs_valid(const Transaction& tx, std::string* error = 0);
...
#define CHECKED_GET_SPECIFIC_VARIANT(variant_var, specific_type, variable_name, fail_return_val) \
  if(variant_var.type() != typeid(specific_type)) { return fail_return_val; } \
