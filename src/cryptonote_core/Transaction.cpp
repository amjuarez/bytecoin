...
namespace CryptoNote {
  ...
  size_t Transaction::addOutput(uint64_t amount, const std::vector<AccountAddress>& to, uint32_t requiredSignatures, uint32_t term) {
    checkIfSigning();
  ...
  std::vector<uint8_t> Transaction::getExtra() const {
    if (transaction.signatures.empty()) {
